...
public sealed class A_Mqtt_Read_Only_Client
{
    ...
    public async Task Should_Iterate_When_Subscribing()
    {
        ...
        {
            await foreach (var item in result.Value!.GetNextAsync(TimeSpan.FromSeconds(10)).WithCancellation(_cancellationToken)
    ...
    private static async Task<string> GetPublishedMessages(ISubscription subscription, CancellationToken cancellationToken)
    {
        ...
        await foreach (var item in subscription.GetNextAsync(TimeSpan.FromMilliseconds(100)).WithCancellation(cancellationToken)
                           .ConfigureAwait(false))
